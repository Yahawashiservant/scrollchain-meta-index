    ...
    return (<Card className="overflow-hidden">
      <CardHeader className="pb-0">
        <div className="flex justify-between items-center">
          ...
          <Badge className="ml-2 px-4 py-2 flex items-center justify-center whitespace-nowrap text-lg font-semibold" variant="outline">
      ...
      <CardContent className="pt-6">
        <div className="relative rounded-lg overflow-hidden bg-gradient-to-br from-primary/5 to-primary/10 border">
          ...
          <div className="aspect-[4/3] md:aspect-[16/9] w-full relative">
            <Image src={uri ?? ""} alt="NFT Image" fill className={cn("object-cover transition-opacity duration-500", isImageLoading ? "opacity-0" : "opacity-100")} onLoad={() => setIsImageLoading(false)}/>
          </div>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-4 items-center">
          <Button className="w-full sm:w-auto gap-2 relative overflow-hidden group" size="lg" onClick={handleMint} disabled={isMinting}>
            <span className={cn("flex items-center gap-2 transition-transform duration-300", isMinting ? "translate-y-10" : "")}>
              <PlusCircle className="h-[18px] w-[18px]"/>
              Mint New NFT
            </span>
            <span className={cn("absolute inset-0 flex items-center justify-center transition-transform duration-300", isMinting ? "translate-y-0" : "translate-y-10")}>
          ...
          {transactionUrl && (<Button variant="outline" size="lg" className={cn("gap-2 w-full sm:w-auto relative overflow-hidden transition-all duration-500", "border-green-400 text-green-700 hover:bg-green-50", "animate-in fade-in duration-700")}>
              <Link href={transactionUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 w-full sm:w-auto">
                {showSuccess ? (<>
                    <div className="absolute inset-0 bg-gradient-to-r from-green-400 to-green-600 opacity-10" style={{
