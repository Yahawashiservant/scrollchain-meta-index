...
export function CM(seed, m) {
    return ((seed % m) + m) % m;
}
// sacred‐angle 369° sampler
export function GS(base) {
    const out = [];
    for (let i = 0; i < C.D; i++) {
...
export function NA(seed, factor) {
    let total = 0;
    for (let pass = 0; pass < C.Ω; pass++) {
