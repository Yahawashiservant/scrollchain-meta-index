...
class FieldOrchestrator extends EventEmitter {
  ...
  async initNats() {
    if (!this.natsUrl) {
      throw new Error('NATS_URL not provided. Unable to orchestrate agents without mesh connectivity.');
    }
    this.natsConnection = await connect({ servers: this.natsUrl, name: this.orchestratorId });
