...
contract TeleportationNFT_144 is ERC721 {
    uint256 public nextId;
    uint256 public constant DIMENSION_ID = 144;
    
    constructor() ERC721("TPT144", "TPT144") {}
    
    function mint(address to, uint256 tone, bytes32 dimID, bytes32 forkHash) external {
      uint256 id = nextId++;
      _safeMint(to, id);
      emit Teleported(to, id, tone, dimID, forkHash);
      emit DimensionalShift(to, id, DIMENSION_ID, 144 * 1000);
    }
    ...
    function batchMint(address[] calldata recipients, uint256[] calldata tones) external {
      ...
      for(uint i = 0; i < recipients.length; i++) {
        ...
        emit Teleported(recipients[i], id, tones[i], bytes32(uint256(144)), bytes32(uint256(id)));
      }
    }
